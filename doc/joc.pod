...
Default is C<openconnect>, i.e. it will typically use the system default.
...
Command to execute for `openconnect`. Defaults to (surprise!)
`openconnect`, but can be set to a custom location (e.g. a newer
version, like `/opt/openconnect-7.07/bin/openconnect`).
